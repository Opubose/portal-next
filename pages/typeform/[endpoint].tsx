...
export default function TypeformApplicationWidgetView() {
  ...
  useEffect(() => {
    const navbarObj = (document.getElementById('portal-navbar') ||
      document.getElementById('portal-navbar-mobile'))!;
  ...
  const { data, error, isLoading } = useQuery({
    queryKey: ['widgetViewUserData', router.query.endpoint],
    queryFn: async () => {
      return await gqlQueries.getUserDataForWidgetView({
        where: {
          endpoint: { equals: router.query.endpoint as string },
        },
      });
    },
  });
  ...
  return (
    <Widget
      className="h-screen"
      id={data!.findFirstTypeformApplication.typeformId}
      hidden={{

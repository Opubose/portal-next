...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  await queryClient.prefetchQuery({
    queryKey: ['fillApplicationPage'],
    queryFn: async () => {
      return await gqlQueries.getSingleApplicationData({
        where: {
          id: { equals: ctx.query.id! as string },
        },
      });
    },
  });
...
export default function FillApplicationPage() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      return await gqlQueries.getSingleApplicationData({
        where: {
          id: { equals: router.query.id! as string },
        },
      });
    },
  ...
  return (
    <ApplicationForm
      ...
      onSubmit={async (formData) => {
        ...
        alert(
          'Your submission is successfully recorded. A confirmation email will be sent to you shortly',
        );

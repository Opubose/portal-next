...
interface MakeUserParticipantCardProps {
  firstName: string;
  lastName: string;
  netid: string;
  profileId: string;
...
export default function MakeUserParticipantCard({
  firstName,
  lastName,
  netid,
  profileId,
  divisions,
}: MakeUserParticipantCardProps) {
      ...
      {({ open }) => (
        <>
          ...
          <Disclosure.Panel className="px-4 pt-4 pb-2 text-sm text-gray-100">
            ...
            <Button
              onClick={async () => {
                await gqlQueries.addParticipantToDivision({
                  where: {
                    profileId,
                  },
                  create: {
                    divisions: {
                      connect: divisions.map((division) => ({
                        id: division.id,
                      })),
                    },
                    profile: {
